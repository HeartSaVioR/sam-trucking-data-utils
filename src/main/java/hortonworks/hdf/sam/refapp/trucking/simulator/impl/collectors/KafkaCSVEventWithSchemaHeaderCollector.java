...
public class KafkaCSVEventWithSchemaHeaderCollector extends BaseTruckEventCollector {
  ...
  @Override public void onReceive(Object event) throws Exception {
  ...
  private void sendTruckSpeedEventToKafka(MobileEyeEvent mee) {
    ...
    logger.debug(
        "Creating truck geo event[" + eventToPass + "] for driver[" + mee.getTruck().getDriver()
            .getDriverId() + "] in truck [" + mee.getTruck() + "]");

    try {
      final Callback callback = new MyProducerCallback();
      Iterable<Header> kafkaHeaders = createKafkaHeaderWithSchema(
          TruckSchemaConfig.KAFKA_RAW_TRUCK_SPEED_EVENT_SCHEMA_NAME);
      ProducerRecord<String, String> data = new ProducerRecord<String, String>(
          TruckSchemaConfig.KAFKA_RAW_TRUCK_CSV_EVENT_TOPIC_NAME, null, driverId, eventToPass,
          kafkaHeaders);
      //logger.debug("Truck Speed Kafka Record with Header is: " + data);
      kafkaProducer.send(data, callback);
    } catch (Exception e) {
      logger.error("Error sending csv geo event[" + eventToPass + "] to  Kafka topic["
          + TruckSchemaConfig.KAFKA_RAW_TRUCK_CSV_EVENT_TOPIC_NAME + "]", e);
    }
  ...
  private void sendTruckEventToKafka(MobileEyeEvent mee) {
    ...
    String driverId = String.valueOf(mee.getTruck().getDriver().getDriverId());
    logger.debug(
        "Creating  truck speed event[" + eventToPass + "] for driver[" + mee.getTruck().getDriver()
            .getDriverId() + "] in truck [" + mee.getTruck() + "]");

    try {
      final Callback callback = new MyProducerCallback();
      Iterable<Header> kafkaHeaders = createKafkaHeaderWithSchema(
          TruckSchemaConfig.KAFKA_RAW_TRUCK_GEO_EVENT_SCHEMA_NAME);
      ProducerRecord<String, String> data = new ProducerRecord<String, String>(
          TruckSchemaConfig.KAFKA_RAW_TRUCK_CSV_EVENT_TOPIC_NAME, null, driverId, eventToPass,
          kafkaHeaders);
      ...
      logger.error("Error sending csv speed event[" + eventToPass + "] to  Kafka topic["
          + TruckSchemaConfig.KAFKA_RAW_TRUCK_CSV_EVENT_TOPIC_NAME + "]", e);
    }

  }

  private Iterable<Header> createKafkaHeaderWithSchema(String schema) {
